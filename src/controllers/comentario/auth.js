...
router.post('/:questaoId', async(req, res) => {
    try{
        ...
        const comentario = await Comentario.create({...req.body, criadoPor: req.userId});
        ...
        console.log(err);
        return res.status(400).send({err: 'Erro ao criar comentario'});
    }
});
...
router.put('/:comentarioId', async(req, res) => {
    try{
        ...
        console.log(err);
        return res.status(400).send({err: 'Erro ao editar comentario'});
    }
});
...
router.delete('/:comentarioId', async(req, res) => {
    try{
        ...
        console.log(err);
        return res.status(400).send({err: 'Erro ao deletar comentario'});
    }
})
